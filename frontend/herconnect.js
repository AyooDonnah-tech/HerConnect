...
const events = [
  { title: 'Tech Innovation Summit', date: 'Jan 28, 2025', location: 'Virtual', category: 'Technology', attendees: 342 },
  { title: 'Leadership Masterclass', date: 'Feb 5, 2025', location: 'New York', category: 'Professional', attendees: 89 },
  { title: 'Alumni Gala Dinner', date: 'Feb 14, 2025', location: 'London', category: 'Social', attendees: 210 },
  { title: 'Startup Pitch Night', date: 'Mar 1, 2025', location: 'San Francisco', category: 'Entrepreneurship', attendees: 156 }
];

const members = [
  { name: 'Sarah Chen', role: 'VP Engineering @ Meta', year: '2015', location: 'San Francisco' },
  { name: 'Marcus Johnson', role: 'Founder @ NovaTech', year: '2012', location: 'Austin' },
  { name: 'Priya Sharma', role: 'Director of AI @ Google', year: '2016', location: 'London' },
  { name: 'David Okonkwo', role: 'Managing Partner @ Apex Capital', year: '2010', location: 'Lagos' }
];
    ...
    <section class="py-16 bg-surface">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
          ${members.map((m, i) => {
            const colors = ['from-brand to-teal', 'from-teal to-gold', 'from-gold to-brand-light', 'from-brand-light to-teal'];
            return `
<div class="card-hover bg-white rounded-2xl p-6 border border-gray-100 anim-fade-up delay-${i+1}"
onclick="navigate('directory')"
...
function render() {
  ...
  switch(state.currentPage) {
    ...
    case 'directory': pageContent = renderDirectoryPage(); break;
    case 'events': pageContent = renderEventsPage(); break;
    case 'mentorship': pageContent = renderGenericPage('Mentorship Program', 'Connect with experienced mentors who can guide your career journey. Coming soon!'); break;
    case 'jobs': pageContent = renderGenericPage('Job Board', 'Exclusive career opportunities from alumni-led companies. Coming soon!'); break;
    default: pageContent = renderHomePage();
  }

  document.getElementById('app').innerHTML = renderNav() + `<main>${pageContent}</main>` + renderFooter();
  lucide.createIcons();
}
